...
# ======================================================
# 🔑 Cookie 区域 (注意：Cookie 如果变了，需要重新 Build)
# ======================================================
...
# 2. 安装基础依赖 和 Chrome (使用最稳健的直接安装法)
# 合并成一步执行，确保 wget 存在后才使用它
...
# 3. 安装 Python 依赖
RUN pip install --no-cache-dir requests selenium undetected-chromedriver lxml

# 4. 【核心功能】启动时自动更新代码
# 每次容器启动，都会先去下载最新的 main.py，然后再运行
# ⚠️ 请确保下面的 URL 是正确的 Raw 链接
CMD ["sh", "-c", "wget -O main.py https://raw.githubusercontent.com/wh1813/my-doc-reader/main/main.py && python main.py"]
